...
const isValidFindSimilarArgs = (args) => typeof args === 'object' &&
    args !== null &&
    typeof args.question === 'string' &&
    (args.limit === undefined || typeof args.limit === 'number') &&
    (args.threshold === undefined || typeof args.threshold === 'number');
const isValidAddExampleArgs = (args) => typeof args === 'object' &&
    args !== null &&
    typeof args.question === 'string' &&
...
class queryAssistant {
    ...
    async generateEmbedding(text) {
        if (!this.openai) {
            throw new McpError(ErrorCode.InternalError, 'OpenAI API key not configured. Please provide it via --openai-key argument or set OPENAI_API_KEY environment variable.');
        }
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to generate embedding: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    async addTrainingExample(question, query, metadata) {
        // Check for duplicates based on question and query
        const existingExample = this.trainingData.find(example => example.question.toLowerCase().trim() === question.toLowerCase().trim() &&
            example.query.toLowerCase().trim() === query.toLowerCase().trim());
        if (existingExample) {
            throw new McpError(ErrorCode.InvalidParams, `Duplicate training example found. Existing example ID: ${existingExample.id}`);
        }
    ...
    async findSimilarExamples(question, limit = 3, threshold = 0.7) {
        if (!this.vectorIndex || this.trainingData.length === 0) {
            return [];
        }
    ...
    async handleFindSimilarQueries(args) {
        if (!isValidFindSimilarArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid arguments for find_similar_queries');
        }
        try {
...
${item.example.metadata?.complexity ? `Complexity: ${item.example.metadata.complexity}` : ''}`;
    ...
    async handleAddTrainingExample(args) {
        if (!isValidAddExampleArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid arguments for add_training_example');
        }
        try {
    ...
    async handleListTrainingExamples(args) {
...
${examples}`;
    ...
    async handleRemoveDuplicates(args) {
        try {
